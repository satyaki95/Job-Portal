...
export default function EmployerDashboard() {
  ...
  return <main className={s.container}><div className={s.contentWrapper}>
    ...
    <div className={s.statsGrid}>{cards.map(({ label, value, icon: Icon, colors }) => <button key={label} type="button" onClick={label === "Total Applicants" ? () => navigate("/employer/applicants") : undefined} className={`${s.statCard} text-left`}><div className={s.statCardOverlay}></div><div className={s.statCardContent}><div className={s.statCardTextContainer}><p className={s.statCardLabel}>{label}</p><p className={s.statCardValue}>{value ?? 0}</p></div><div className={`${s.statCardIconWrapper} ${colors.bgLight} bg-linear-to-br ${colors.gradient}`}><Icon className={s.statCardIcon} strokeWidth={1.8} /></div></div></button>)}</div>
