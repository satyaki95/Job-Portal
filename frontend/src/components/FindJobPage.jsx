...
const FindJobPage = () => {
  ...
  const toggleSaveJob = async (jobId) => {
    try {
      ...
      const res = await fetch(`${baseURL}/api/saved/job/${jobId}`, {
        method: "POST",
        headers: {
  ...
  const confirmApply = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${baseURL}/api/application/apply/${confirmToast.jobId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    const fetchAppliedJobs = async () => {
      try {
        ...
        const res = await fetch(`${baseURL}/api/application/user`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      const res = await fetch(`${baseURL}/api/job?${params.toString()}`);
      const data = await res.json();
  ...
  const renderLogo = (job) => {
    ...
    const isBroken = imgErrors[job.id] || !logoUrl || logoUrl === `${baseURL}`;
      ...
      <div className={s.mainContent}>
        {!showFilters && (
          <button
            ...
            className={s.mobileShowFiltersButton}
          >
            <Filter className={s.mobileShowFiltersIcon} />
            Show Filters
          </button>
        )}
        ...
        <div className={s.contentGrid}>
          {/* Filters sidebar */}
          <div
            className={showFilters ? s.sidebar : s.sidebarHidden}
            ref={filtersRef}
          >
