...
const CompanyPage = () => {
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        const response = await axios.get(`${baseURL}/api/interview/companies`);
    ...
    const fetchQuestions = async () => {
      ...
      try {
        const response = await axios.get(
          `${baseURL}/api/interview/companies/${selectedCompany}`,
        );
  ...
  // to save or unsave the question
  const toggleSave = async (id, openSaved = false) => {
    try {
      ...
      const res = await fetch(
        `${baseURL}/api/saved/question/${id}?type=interview`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
          ...
          <>
            <div className={s.questionsGrid}>
              {questionsToShow.map((question, index) => {
                ...
                const questionDate =
                  question.postDate || question.postdate || question.createdAt;
